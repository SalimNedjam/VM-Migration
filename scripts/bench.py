...
class VM:
    ...
    def __init__(self, name: str, mem_size: int, is_vanilla: bool, extra: Optional[List[str]] = None):
        ...
        if name == "source":
            self._cmd += ["-drive", "file=/home/dev/git/vm-migration-fingerprint/qemu-5.2.0/imgs/foobar.qcow2,if=virtio"]
            self._cmd += ["-net", "nic"]
            self._cmd += ["-net", "user,hostfwd=tcp::2222-:22"]
        else:
...
def main(argv):
    ...
    src_out = os.path.dirname(os.path.abspath(__file__)) + "/src_out.json"
    dst_out = os.path.dirname(os.path.abspath(__file__)) + "/dst_out.json"
    timeout = "3600"
    vm_memory = "5120"
    stress_pages = "2009600"
    stress_rate = "0"
    migration_target = "tcp:127.0.0.1:4444"
    try:
    ...
    if is_vanilla:
        ...
        f = open("modified.out", "a")
    param_list = [is_vanilla, vm_memory, stress_rate, stress_pages]
    f.write(str(param_list)+'\n')

    f.close()
    vm_dst = VM("dest", vm_memory, is_vanilla, ["-incoming", f"{migration_target}"])
    vm_src = VM("source", vm_memory, is_vanilla)
    j_dst = vm_dst.run()
    j_src = vm_src.run()
    time.sleep(15)
    if not stress_rate == "0":
        s = pxssh.pxssh()

        if not s.login('localhost', 'vm', 'vm', port=2222):
            print("SSH session failed on login.")
            print(str(s))
        else:
            print("Connected to SSH.")
            print("Run stress memory script.")
            s.sendline('/home/vm/dirty -r '+stress_rate+' -s '+stress_pages+' -t '+timeout+' &disown')
            s.prompt()
            s.logout()
    ...
    #vm_src.qmp.send({"execute": "human-monitor-command", "arguments": {"command-line": "migrate_set_capability auto-converge on"}})  
    vm_src.qmp.send({"execute": "human-monitor-command", "arguments": {"command-line": "migrate_set_parameter fingerprint-path " + src_out}})
    vm_dst.qmp.send({"execute": "human-monitor-command", "arguments": {"command-line": "migrate_set_parameter fingerprint-path " + dst_out}})
    vm_src.qmp.send({"execute": "human-monitor-command", "arguments": {"command-line": "info migrate_capabilities"}})
    vm_src.qmp.send({"execute": "human-monitor-command", "arguments": {"command-line": "info migrate_parameters"}})
    vm_dst.qmp.send({"execute": "human-monitor-command", "arguments": {"command-line": "info migrate_parameters"}})

    if need_migrate_disk:
        vm_src.qmp.send({
            "execute": "migrate",
            "arguments": {
                "uri": "tcp:127.0.0.1:4444",
                "blk":True,
                "inc":True
            }
        })
    ...
    vm_src.qmp.send({"execute": "human-monitor-command", "arguments": {"command-line": "info migrate"}})
    time.sleep(1)
    vm_dst.qmp.send({"execute": "human-monitor-command", "arguments": {"command-line": "info status"}})
    time.sleep(1)
    vm_src.quit()
    vm_dst.poweroff()
    j_src.join()
